...
async function book_session(mentee_id, session_data) {
    ...
    await availability_service.get_availability(mentor_id, date, s, e);
    const status = 'pending';
    const start_datetime = `${date} ${s}`;
    const end_datetime = `${date} ${e}`;
...
async function confirm_booking(mentor_id, session_id) {
    ...
    const meet_link = await google_service.createMeetLink(new Date(start).toISOString(), new Date(end).toISOString());
